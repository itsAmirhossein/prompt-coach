...
*v0.3.0 (2026-08-03) — **Batch & file input** (`/batch`, skill `prompt-batch`). A dispatcher, not a new analyzer: it determines the structure of whatever it is handed (a single prompt, many pasted prompts, a file of prompts, a Claude Code transcript, an exported chat, or a CLAUDE.md-shaped context file), segments it, and routes each item to the existing capability that fits — a prompt to `prompt-improve`, a conversation to `prompt-retro`, a context file to `prompt-audit` — running that capability's own procedure unchanged. Design decisions worth recording: (a) **one new analytical step only** — the cross-item summary (recurring rule IDs with counts, top-2 patterns, offered artifacts), computed over the per-item findings, so nothing is re-analyzed; this generalizes `/retro`'s existing "many items → grounded findings → consolidated artifacts" skeleton to arbitrary input sources. (b) **Identical per-item behavior** — the only adaptation for many items is deferring each capability's terminal choose-and-run / apply-diff interaction and consolidating it once at the end, which is exactly the review posture `/improve --file` and `/retro` already use (the "execute into this session" branch was always conditional on the item being meant for this session). (c) **No parser change** — `parse_transcript.py` already resolves a `--session <path>` to an arbitrary file, so Claude Code transcript files route straight through; non-Claude exports are segmented agent-side, keeping the change purely additive (one command + one skill + a grading fixture). (d) **Future-proof by construction** — any capability added later is reachable by adding a row to the routing table, satisfying the spec's "reuse any future capability automatically."*

*v0.4.0 (2026-08-06) — **`/batch` renamed to `/check`** (skill `prompt-batch` → `prompt-check`), no deprecation alias. Rationale: "batch" described the incidental property (plurality) rather than the essential one (routing) — the command's identity is "hand me anything and I'll run the right review," and it accepts a single item. Two observed failures of the old name: the product owner himself asked why the command existed, and `/batch` collided with a user's local batch-orchestration command and misfired. No alias because the installed base was days old and an alias would have preserved exactly that collision. Also: **`/retro --file <path>`** — retro now accepts a conversation file directly (`.jsonl` via the parser, which already resolved arbitrary paths; text/markdown exports parsed agent-side with an honest no-token-data cost line), and `/check`'s conversation routing row now simply delegates to it. Division of labor: know what it is and it's one thing → the specialist command; plural, mixed, or unidentified → `/check`. `/retro` keeps its standalone door because its main flow (`--current`, no input at all) has nothing for a dispatcher to route.*
